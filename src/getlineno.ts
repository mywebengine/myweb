...
function createLineNo(url: string, html: string, $src: Node) {
	let line = 1;
	let $i = $src;
	const $parent = $i.parentNode;
	const $p = new Array<Node>();
	do {
		//////////////////////
		if ($i.nodeType === 1) {
			const localName = ($i as HTMLElement).localName;
			...
			//console.log(1, line);
			//alert(1)
			($i as HTMLElement).setAttribute(lineNoAttrName, `${url}:${line}`);
			...
			if ($i.nodeName === "TEMPLATE" && ($i as HTMLTemplateElement).getAttribute(config.hideName) !== null) {
				$p.push($i);
				$i = ($i as HTMLTemplateElement).content.firstChild as HTMLElement;
				continue;
			}
		}
		if ($i.parentNode === $parent) {
...
function getLineNo($e: HTMLElement) {
	return $e.getAttribute(lineNoAttrName);
}

const my = (self as unknown as {my: {createLineNo: typeof createLineNo; getLineNo: typeof getLineNo}}).my;
