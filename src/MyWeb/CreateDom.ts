...
export abstract class CreateDom extends Base_MyWeb {
	createSrc($e: HTMLElement, descr: Descr | null, asOneIdx: Map<string, number> | null, idx: Map<string, number> | null) {
			...
			const src: Src =
				descr.attr === null
					...
					: new Src(
							...
							true,
							isHide,
							descr.isHasScope ? this.getScopeReact({}) : null,
							null,
							null,
							new Cache()
					  );
...
*/
	...
	createDescr($e: HTMLElement, srcId: number) {
		...
		for (const [str, expr] of attr) {
			const rc = this.context.commandWithArgsByStr.get(str) as CommandWithArgs;
			// if (rc === null) {
			// 	continue;
			// }
...
*/
	...
	prepare$src($i: Node, isLinking: boolean) {
		//$i = this.context.rootElement, isLinking) {//todo это не будет работать если после фора идет вставка на много тегов
		const $parent = $i.parentNode;
		const $p = [];
		const idAlias = new Map<string, Descr | number>(); //todo разнотипный мап!? ---- это вообще нужно убрать - это про старый SSR
		do {
			//////////////////////
			switch ($i.nodeType) {
				case 1:
					...
					$i = this.prepare$src_createSrc($i as HTMLElement, idAlias, isLinking);
					break;
				case 3:
					$i = this.replaceTextBlocks($i as Text) as Node;
					break;
			}
	...
	joinText($e: HTMLElement | DocumentFragment) {
	...
	private createAttr($e: HTMLElement) {
	...
	private prepare$src_createSrc($e: HTMLElement, idAlias: Map<string, Descr | number>, isLinking: boolean) {
		if (!isLinking) {
			this.createSrc($e, null, null, null);
			return $e;
		}
		//todo --
		const src = this.prepare_getSrc($e, idAlias as Map<string, Descr>); //todo
		...
		for (const str of src.descr.attr.keys()) {
			...
			if (asOneIdx !== null) {
				const aIdx = idAlias.get(asOneIdx) as number; //todo
				if (aIdx === undefined) {
	...
	private prepare_getSrc($e: HTMLElement, idAlias: Map<string, Descr>) {
...
//						$i = null;
	...
	private replaceTextBlocks($src: Text) {
		...
		const text = $src.textContent || "";
		let i = text.indexOf("${");
		if (i === -1 || ($src.parentNode as HTMLElement).nodeName === "SCRIPT") {
			return $src;
		}
		const $parent = $src.parentNode as HTMLElement; //!!
