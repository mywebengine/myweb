...
export abstract class Eval2 extends ShowHide {
	eval2(req: Req, $src: HTMLElement, isReactive: boolean) {
		...
		const cache = (this.context.srcById.get(cacheSrcId) as Src).cache; //!!
	...
	eval2Execute(req: Req, $src: HTMLElement) {
		const func = this.getEval2Func(req, req.expr);
		return func.apply($src, [req.scope, req.event]).catch((err: Error) => {
			throw this.getError(err, req.$src, req);
		});
	}

	async q_eval2(req: Req, arr: Q_arr[], isLast: Set<number>) {
		return this.q_getEval2Func(req, req.expr)
			...
			.then(values => Promise.all(values as unknown[]))
			.catch(err => {
				throw this.getError(err as Error, req.$src, req);
			});
	}

	//const commentRe = /\/\/.+?(\n|$)/g;
	private getEval2Func(req: Req, expr: string) {
		//expr = expr.trim();
		const cacheKey = expr;
...
return typeof _tpl_res === "function" ? _tpl_res.apply(this, [_tpl_scope, _tpl_event]) : _tpl_res`;
	...
	private q_getEval2Func(req: Req, expr: string) {
		//expr = expr.trim();
...
function isNeedRet(expr: string) {
	if (expr === "") {
		return false;
	}
...
function clearBlock(str: string, begin: string, end: string) {
