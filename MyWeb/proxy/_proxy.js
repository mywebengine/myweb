...
self._testVars = function() {
	const v = new Set(Array.from(my.context.varIdByVar.values()));
	for (const [vId, srcIds] of my.context.srcIdsByVarId) {
		let fv;
		if (!v.has(vId)) {
			let f;
			for (const vvId of v) {
				const vIdByProp = my.context.varIdByVarIdByProp.get(vvId);
	...
	for (const vId of my.context.varIdByVar.keys()) {
		const s = my.context.srcIdsByVarId.get(vId);
		if (!s) {// || !s.has(sId)) {
			continue;
		}
		for (const sId of s) {
		...
		const vIdByProp = my.context.varIdByVarIdByProp.get(vId);
		if (vIdByProp !== undefined) {
			for (const pId of vIdByProp.values()) {
				const propS = my.context.srcIdsByVarId.get(pId);
...
function addVar(t, n, v, $src) {
	const tId = my.context.varIdByVar.get(t),
		src = my.context.srcBy$src.get($src),
		sId = src.id,
		descr = src.descr;
//	if (!sId) {
...
console.warn(445435345, n, typeof n, t, n, v, $src);
		...
		if (tId) {
			const s = my.context.srcIdsByVarId.get(tId);
			if (s !== undefined) {
				s.add(sId);
			} else {
				my.context.srcIdsByVarId.set(tId, new Set([sId]));
//todo
//console.warn(-1, tId);
			}
//1
			...
			const vIdByProp = my.context.varIdByVarIdByProp.get(tId);
			if (vIdByProp !== undefined) {
...
//1
					...
					const s = my.context.srcIdsByVarId.get(propId);
					if (s !== undefined) {
						s.add(sId);
						return;
					}
					my.context.srcIdsByVarId.set(propId, new Set([sId]));
//console.log(-2, propId, tId, n, v);
					return;
				}
				const newPropId = getNewId();
//console.log(-33, my.context.varIdByVarIdByProp.get(tId));
//console.log(-3, newPropId, tId, n, v);
//alert(1);
//1
				vIdByProp.set(n, newPropId);
				my.context.srcIdsByVarId.set(newPropId, new Set([sId]));
				descr.varIds.add(newPropId);
				return;
			}
			const newPropId = getNewId();
			my.context.varIdByVarIdByProp.set(tId, new Map([[n, newPropId]]));
			my.context.srcIdsByVarId.set(newPropId, new Set([sId]));
//console.log(-44, new Set(my.context.varIdByVarIdByProp.get(tId)));
...
//1
		...
		const nId = getNewId();
		my.context.varIdByVar.set(t, nId);
		my.context.varById.set(nId, t);
		my.context.srcIdsByVarId.set(nId, new Set([sId]));
//console.log(-5, nId);
//1
		descr.varIds.add(nId);

		const newPropId = getNewId();
		my.context.varIdByVarIdByProp.set(nId, new Map([[n, newPropId]]));
		my.context.srcIdsByVarId.set(newPropId, new Set([sId]));
...
//1
	...
	const vId = my.context.varIdByVar.get(v);
	if (vId) {
		const s = my.context.srcIdsByVarId.get(vId);
		if (s !== undefined) {
			s.add(sId);
		} else {
			my.context.srcIdsByVarId.set(vId, new Set([sId]));
//console.log(-9, sId);
		}
//1
	...
	my.context.varIdByVar.set(v, newValId);
	my.context.varById.set(newValId, v);
	my.context.srcIdsByVarId.set(newValId, new Set([sId]));
//console.log(-10, sId);
//1
	descr.varIds.add(newValId);
}
function setVal(t, n, v, oldV) {//!! data.arr.unshift(1); data.arr.unshift(2); - если так сделалть, то после первого - будут удалены my.context.varIdByVar.get(oldId), что приведет к тому что все пойдет по ветке !oldId - непонятно нужно ли что-то с этим делать??
	const tId = my.context.varIdByVar.get(t);
...
//		oId = oldScalarId ? my.context.varIdByVar.get(oldV) : 0;
		oldScalarId = vIdByProp !== undefined && vIdByProp.get(n) || 0,
		oId = oldScalarId !== 0 && my.context.varIdByVar.get(oldV) || 0;
	if (my.debugLevel === 2) {
...
/*!!нет в этом смысла
	if (!oId) {//если в разметке нет этого свойства, а оно используется в расчётах - для того чтобы на следующем круге понять что оно уже использовалось и не нужно чистить кэш по условию !oId
		if (isScalarType.has(typeof v) || v === null) {
			...
			my.context.varIdByVar.set(v, newId);
			my.context.varById.set(newId, v);
		}
	}*/
/*--
	if (!oId) {
		const s = my.context.srcIdsByVarId.get(tId);//для push - нового элемента нет, а обновить надо - это актуально когда нет if .length
		if (s) {
//			for (const sId of s) {//
...
//!!todo GC
		if (vIdByProp.size === 0) {
			my.context.varIdByVarIdByProp.delete(tId);
		}
	}
//	_setVal(t, n, oldV, my.context.srcIdsByVarId.get(oId || tId), oId);
//}
//function _setVal(t, n, oldV, s, oId) {
	const s = my.context.srcIdsByVarId.get(oId || tId);
	if (s === undefined) {
//!!todo
...
//console.error("_setVal", t, n, oldV, s, oId);
	const toRender = new Set(s),
		toClear = new Set(),
		srcBy$src = my.context.srcBy$src;//s);
	for (const sId of s) {
		const $i = my.context.$srcById.get(sId);
...
for (const sId of toClear) {
	console.log(2, sId, my.context.$srcById[sId]);
}*/
	if (oId !== 0) {
		const deletedVarId = new Set();
		for (const sId of toClear) {
			const c = my.context.srcById.get(sId).cache;
//			if (c === null) {
//				continue;
//			}
			c.value = new Map();
...
//todo
			requestIdleCallback(() => {
				for (const d of my.context.descrById.values()) {
					if (d.varIds === null) {
						continue;
					}
...
function setInnerSrcIdSetBy$src(toClear, $i) {
	const srcBy$src = my.context.srcBy$src,
		$parent = $i.parentNode;
	do {
		const iSrc = srcBy$src.get($i);
...
function decVar(t, n, v, sId, vId, deletedVarId) {
	if (vId === 0) {
		if (isScalarType.has(typeof v) || v === null) {
			const vIdByProp = my.context.varIdByVarIdByProp.get(my.context.varIdByVar.get(t));
			if (vIdByProp !== undefined) {
				vId = vIdByProp.get(n);
				if (vId === undefined) {
					vId = 0;
			...
			vId = my.context.varIdByVar.get(v);
			if (vId === undefined) {
				vId = 0;
			}
		}
...
//console.log("DEL", vId);
	...
	my.context.srcIdsByVarId.delete(vId);
	if (isScalarType.has(typeof v) || v === null) {
		const vIdByProp = my.context.varIdByVarIdByProp.get(vId = my.context.varIdByVar.get(t));
		if (vIdByProp === undefined) {
			return;
		}
		vIdByProp.delete(n);
		if (vIdByProp.size === 0) {
			my.context.varIdByVarIdByProp.delete(vId);
		}
		return;
	}
	...
	my.context.varIdByVar.delete(v);
	my.context.varById.delete(vId);
//!!	my.context.varIdByVarIdByProp.delete(vId);
	const vIdByProp = my.context.varIdByVarIdByProp.get(vId);
...
//!! сейчас там то что не используется, - по какойто причине в get прокси запрашивается "then" - хотя в шаблоне нет такого запроса
	if (vIdByProp !== undefined) {
		my.context.varIdByVarIdByProp.delete(vId);
		for (const pId of vIdByProp.values()) {
			my.context.srcIdsByVarId.delete(pId);
		}
	}
}
