...
//import {type_isLast} from "../../render/IsLast.js";
import {type_q_arr} from "../../render/Q_arr.js";
import {Animation} from "../../render/Animation.js";
import {type_renderRes} from "../../render/RenderRes.js";

import {p_target, visibleScreenSize, defIdleCallbackOpt, foreachCmdName, renderBatchSize} from "../../config/config.js";
//import {type_asOneIdx} from "../../description/type.js";
import {getNewId, getSrcId, get$els, get$first, getNextStr} from "../../description/descr.js";
	...
	return q_eval2(req, arr, isLast)
		.then(vals => {
			...
			for (let i = 0; i < arrLen; i++) {
				if (!isLast.has(i)) {
					res[i] = foreach_render(my.createReq(arr[i].$src, req.str, req.expr, arr[i].scope, req.sync), vals[i]);
				}
			}
			return res;
		});
...
//			}
			...
			req.sync.afterAnimations.add(new Animation(() => q_forRender(req, ctx, ctx.els, elsLen < keysLen ? () => q_add(req, ctx) : null), req.sync.local, 0));
			return res;
		}
	}
	if (elsLen === keysLen) {
...
//		for (let j = ctx.els[i].length - 1; j > -1; j--) {
	...
	req.sync.afterAnimations.add(new Animation(() => q_forRender(req, ctx, ctx.els, null), req.sync.local, 0));
	return res;
/*
...
//		throw new Error("foreach.js");
//	}
	const step = viewSize !== 0 ? Math.ceil(document.scrollingElement.clientHeight * visibleScreenSize / viewSize) : renderBatchSize;
//	if (is$visible(from$last)) {
	if (is$visible(from.$els[from$elsLen - 1])) {
		req.sync.animations.add(new Animation(() => q_addInsert(req, ctx, getSrcId(req.sync.local, sId), keysLen, idx, step), req.sync.local, 0));//!! нельзя не вставить этот элементи двигасться дальше, так что если даже на момент отрисовки его не будет видно, его всё рано нужно вставить
		return;
	}
	req.sync.afterAnimations.add(new Animation(() => q_addDeferred(req, ctx, sId, keysLen, idx, step), req.sync.local, 0));
}
function q_addDeferred(req, ctx, sId, keysLen, idx, step) {
	return new Promise(ricResolve => {//обязательно нужден проимс
		const ricId = requestIdleCallback(() => {
			...
			req.sync.animations.add(new Animation(() => q_addInsert(req, ctx, getSrcId(req.sync.local, sId), keysLen, idx, step), req.sync.local, sId));
			ricResolve();
		}, defIdleCallbackOpt);
...
//		$last = get$last(req, from$last, idx - 1);
		$last = get$last(req, my.ctx.$srcById.get(sId), idx - 1);
	idx += newElsLen;
	if (idx >= keysLen) {
/*
//todo
if (!$last.parentNode) {
	console.error(req, ctx, sId, keysLen, idx, step, $last);
		...
		req.sync.afterAnimations.add(new Animation(() => q_forRenderI(req, ctx, newEls), req.sync.local, 0));
		return;
	}
	...
	for (let $i = newElsLast$els[newElsLast$els.length - 1]; $i !== null; $i = $i.previousSibling) {
		...
		sId = iSrc.id;
		$last.parentNode.insertBefore($fr, $last.nextSibling);
		req.sync.afterAnimations.add(new Animation(() => q_forRenderI(req, ctx, newEls)
...
/*
function get$last(req, $last, lastIdx) {
	const asOneIdx = my.ctx.srcBy$src.get($last).asOneIdx.get(req.str);
	for (let $i = $last.nextSibling; $i !== null; $i = $i.nextSibling) {
		const iSrc = my.ctx.srcBy$src.get($i);
		if (iSrc === undefined || !iSrc.isCmd) {
			continue;
		}
		if (iSrc.asOneIdx === null || iSrc.asOneIdx.get(req.str) !== asOneIdx || getIdx(iSrc, req.str) !== lastIdx) {
		...
		return q_forRenderI(req, ctx, nows)
			.then(() => {
				if (deferreds.length !== 0) {
					req.sync.afterAnimations.add(new Animation(() => q_forRender(req, ctx, deferreds, addF), req.sync.local, 0));
					return;
				}
				if (addF !== null) {
					addF();
...
//console.log(2, deferreds)
	return new Promise(ricResolve => {
		const ricId = requestIdleCallback(() => {
			req.sync.idleCallback.delete(ricId);
			q_forRenderI(req, ctx, deferreds.splice(0, renderBatchSize))
				.then(() => {
					if (deferreds.length !== 0) {
...
//todo  нужно переносить на стр - атрибуты мешаю рендерить фор->инк (1)
	return q_renderTag(arr, req.str, new Set(), req.sync);
}
