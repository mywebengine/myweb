...
import {type_localCounter} from "../../render/LocalCounter.js";
...
//import {type_asOneIdx, type_idx, type_save, type_saveI} from "../../description/type.js";
import {getAttrItAfter, get$els} from "../../description/descr.js";
	...
	return q_eval2(req, arr, isLast)
		.then(vals => {
			...
			for (let i = 0; i < arrLen; i++) {
				if (!isLast.has(i)) {
					res[i] = inc_render(my.createReq(arr[i].$src, req.str, req.expr, arr[i].scope, req.sync), vals[i]);
...
//console.info("inc", req);
	...
	const pos = -1,
		...
		$elsLen = $els.length,
		oldVal = getIdx(my.ctx.srcBy$src.get(req.$src), req.str);
//todo
	for (let i = 0; i < $elsLen; i++) {
		const iSrc = my.ctx.srcBy$src.get($els[i]);
		if (iSrc !== undefined && !req.sync.local.has(iSrc.id)) {
			req.sync.local.set(iSrc.id, type_localCounter());
		}
	}
//console.log(111, req, $els, oldVal, my.ctx.srcBy$src.get(req.$src), include);
//alert(1);
...
//todo так как у респонса неполчится узнать гет ли он - то все добовляем кэш - А при использовании в строке в фетч - он всегда будет разный ...
	...
	const r = getRequest(val, getTopUrl(my.ctx.srcBy$src.get(req.$src), req.str)),
...
//		if (newSrcId !== 0) {
			const vIds = my.ctx.srcById.get(newSrcId).descr.varIds;
			for (const vId of iSrc.descr.varIds) {
				vIds.add(vId);
				my.ctx.srcIdsByVarId.get(vId).add(newSrcId);
/*
				const sIds = my.ctx.srcIdsByVarId.get(vId);
if (sIds === undefined) {
//todo
console.warn("inc.js");
alert(1)
...
function incClear(str, src, incKey) {
	...
	for (const n of getAttrItAfter(src.descr.attr.keys(), str, false)) {
		if (my.ctx.reqCmd.get(n).cmdName !== incCmdName) {
			break;
		}
		incKey = getIdx(src, n);
		if (incKey === undefined) {
...
/*
	const afterAttr = new Map(),
		descr = my.ctx.srcBy$src.get($e).descr,
		attrIt = descr.attr.entries();
	for (let i = attrIt.next(); !i.done; i = attrIt.next()) {
		const [n, v] = i.value,
			cn = my.ctx.reqCmd.get(n).cmdName;
//todo inc args --
//		if (v && ((cn !== incCmdName && cn !== fetchCmdName) || !my.ctx.reqCmd.get(n).args[0]) && cn !== elseCmdName && cn !== defaultCmdName && cn !== onCmdName) {
//todo наверное всё же не так! для иквов фетчей это может стать проблемой
		if (v && cn !== elseCmdName && cn !== defaultCmdName && cn !== onCmdName) {
//todo
console.warn(n, v);
			await eval2(my.createReq($e, n, v, req.scope, req.sync), $e, true);//привязываем к новым тегам команды ДО
		}
		if (n !== req.str) {
			continue;
		}
...
async function renderI(req, $e, $last, h) {
	do {
		const iSrc = my.ctx.srcBy$src.get($e);
...
//	const $fr = cloneNode(req, include.$fr),
	const $fr = include.$fr.cloneNode(true),
		src = my.ctx.srcBy$src.get(req.$src),
		descr = src.descr,
...
//		idx = src.idx !== null ? src.idx : new Map(),
	...
	for (let k, nn, j, i = 0, $i = $fr.firstChild; $i !== null; i++, $i = $i.nextSibling) {
		...
		iSrc.save = createSave(save);
		if (onLen !== 0) {
			for (k = 0; k < onLen; k++) {
				const o = on[k];
				o.cmd.render(my.createReq($i, o.str, o.expr, req.scope, req.sync));
			}
		}
//!!2
		if (my.debugLevel !== 0) {
...
//			const jSrc = my.ctx.srcBy$src.get($v[j]);
//			if (jSrc === undefined) {
//				$s.appendChild($v[j]);
//				continue;
//			}
//			const get$elsByStr = jSrc.descr.get$elsByStr;
			const $j = $v[j],
				get$elsByStr = my.ctx.srcBy$src.get($j).descr.get$elsByStr;
...
//todo , expr, pos
	let count = 0;
	const attrIt = getAttrItAfter(my.ctx.srcBy$src.get($i).descr.attr.keys(), str, false);
	for (let i = attrIt.next(); !i.done; i = attrIt.next()) {
		if (my.ctx.reqCmd.get(i.value).cmdName !== incCmdName) {
			continue;
		}
