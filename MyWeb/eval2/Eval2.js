...
export default class Eval2 extends Dom {
	eval2(req, $src, isReactive) {
		...
		const cacheSrcId = this.getCacheSrcId(req.$src, req.str),
			c = this.ctx.srcById.get(cacheSrcId).cache;
...
//console.log(7777, req.str, req.expr, req.$src, c.value);
		...
		if (isReactive) {
			...
			const val = func
				...
				.catch(err => {
					throw this.getErr(err, req.$src, req);
				});
...
//			proxyStat.value = 0;
	...
	q_eval2(req, arr, isLast) {
		return this.q_getEval2Func(req, req.expr)
			.apply(null, [req, arr, isLast, req.str, setCur$src, proxyStat, this.ctx.srcBy$src])
...
//const commentRe = /\/\/.+?(\n|$)/g;
	...
	getEval2Func(req, expr) {
		expr = expr.trim();
		const cacheKey = expr,
			_f = this.ctx._func.get(cacheKey);
		if (_f !== undefined) {
			return _f;
		}
		if (expr === "") {
			const ff = new func();
			this.ctx._func.set(cacheKey, ff);
			return ff;
		}
//todo
