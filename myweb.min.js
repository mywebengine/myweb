...
["/descr.js", import('data:text/javascript;text,let setReqCmd,type_cache,reqCmd,incCmdName,hideName,setAsOneIdx,getIdx,setIdx,loadingCount,getProxy;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/render/render.js"),self.__i.get("/cache.js"),self.__i.get("/config.js"),self.__i.get("/dom.js"),self.__i.get("/loading.js"),self.__i.get("/proxy.js")]).then(arr=>{let m;m=arr[0];setReqCmd=m.setReqCmd;m=arr[1];type_cache=m.type_cache;m=arr[2];reqCmd=m.reqCmd;incCmdName=m.incCmdName;hideName=m.hideName;m=arr[3];setAsOneIdx=m.setAsOneIdx;getIdx=m.getIdx;setIdx=m.setIdx;m=arr[4];loadingCount=m.loadingCount;m=arr[5];getProxy=m.getProxy;}));export const $srcById=new Map;export const srcById=new Map;export const srcBy$src=new WeakMap;export const descrById=self.mw_descrById||new Map;export let idCurVal=self.mw_idCurVal||0;self.mw_$srcById=$srcById;self.mw_srcById=srcById;export function getNewId(){return++idCurVal}export function createSrc(e,t,r,n){const s=getNewId(),o=e.getAttribute(hideName)!==null;if(t===undefined){t=createDescr(e,s);const c=t.attr!==null?type_src(s,t,true,o,null,null,type_cache()):type_src(s,t,false,o,null,null,null);$srcById.set(s,e);srcById.set(s,c);srcBy$src.set(e,c);return c}t.srcIds.add(s);const c=t.attr!==null?type_src(s,t,true,o,r,n,type_cache()):type_src(s,t,false,o,null,null,null);$srcById.set(s,e);srcById.set(s,c);srcBy$src.set(e,c);if(!c.isCmd){return c}moveLoading(e,s);return c}export function createDescr(e,t){const r=getNewId(),n=createAttr(e);if(n.size===0){const s=type_descr(r,t,null,null);descrById.set(r,s);return s}const s=type_descr(r,t,n,new Set);let o=0;for(const[c,u]of n){const d=reqCmd.get(c);if(d.cmd.get$els){if(s.get$elsByStr===null){s.get$elsByStr=type_get$elsByStr()}s.get$elsByStr.set(c,type_get$elsByStrI(u,o))}o++;if(d.cmd.isCustomHtml&&s.isCustomHtml===false){s.isCustomHtml=true}if(d.cmd.isAsOne){if(s.asOnes===null){s.asOnes=new Set}s.asOnes.add(c)}}descrById.set(r,s);return s}function moveLoading(e,t){const r=loadingCount.get(e);if(r===undefined){return}loadingCount.set(t,r);loadingCount.delete(e)}function type_src(e,t,r,n,s,o,c){return{id:e,descr:t,isCmd:r,isHide:n,asOneIdx:s,idx:o,save:null,cache:c,scopeCache:getProxy({}),isMounted:false}}export function type_asOneIdx(e){return new Map(e)}export function type_idx(e){return new Map(e)}export function type_save(e){if(e===undefined){return new Map}const t=new Map;for(const[r,n]of e){t.set(r,type_saveI(n))}return t}export function type_saveI(e){return new Map(e)}function type_descr(e,t,r,n){return{id:e,sId:t,attr:r,varIds:n,srcIds:new Set([t]),isCustomHtml:false,asOnes:null,get$elsByStr:null}}function type_get$elsByStr(){return new Map}function type_get$elsByStrI(e,t){return{expr:e,pos:t}}export function createAttr(e){const t=new Map,r=e.attributes,n=r.length;for(let e=0;e<n;e++){const s=r[e];if(setReqCmd(s.name)){t.set(s.name,s.value)}}return t}export function getSrcId(t,r){if(srcById.has(r)){return r}for(let e=t.get(r);e!==undefined&&e.newSrcId!==0;e=t.get(r)){r=e.newSrcId}return r}export function get$els(e,t,r){const n=srcBy$src.get(e).descr.attr.keys();let s=n.next();if(r!==""){for(;!s.done;s=n.next()){if(s.value===r){break}}}while(!s.done){const o=s.value,c=t.get(o);if(c!==undefined){return reqCmd.get(o).cmd.get$els(e,o,c.expr,c.pos)}s=n.next()}return[e]}export function get$first(e,t,r){const n=srcBy$src.get(e).descr.attr.keys();let s=n.next();if(r!==""){for(;!s.done;s=n.next()){if(s.value===r){break}}}while(!s.done){const o=s.value,c=t.get(r);if(c!==undefined){return reqCmd.get(r).cmd.get$first(e,r,c.expr,c.pos)}s=n.next()}return e}export function getNextStr(e,t){const r=e.descr.attr.keys();for(let e=r.next();!e.done;e=r.next()){if(e.value!==t){continue}e=r.next();if(e.done){return""}return e.value}return""}export function getAttrAfter(e,t){const r=new Map,n=getAttrItAfter(e.entries(),t,true);for(let e=n.next();!e.done;e=n.next()){r.set(e.value[0],e.value[1])}return r}export function getAttrItAfter(t,r,e){if(r===""){return t}if(e){for(let e=t.next();!e.done;e=t.next()){if(e.value[0]===r){return t}}return t}for(let e=t.next();!e.done;e=t.next()){if(e.value===r){return t}}return t}')],
...
["/loading.js", import('data:text/javascript;text,let isFillingName,isFillingDiv,srcBy$src,is$hide;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/config.js"),self.__i.get("/descr.js"),self.__i.get("/dom.js")]).then(arr=>{let m;m=arr[0];isFillingName=m.isFillingName;isFillingDiv=m.isFillingDiv;m=arr[1];srcBy$src=m.srcBy$src;m=arr[2];is$hide=m.is$hide;}));export const loadingCount=new Map;export async function showLoading(t,i,e="",n){const o=getLoadingKey(t),r=loadingCount.get(o);if(await i()){if(r!==undefined){decLoading(t,e,r)}return}const g=r===undefined?createLoading(o):r;if(n===undefined||n===""){n=-1}if(n<0){toggleLoading(t,"",true,g);if(e!==""){toggleLoading(t,e,true,g)}return}toggleLoading(t,"",true,g);if(e===""){return}setTimeout(async()=>{if(is$hide(t)){return}if(!await i()){toggleLoading(t,e,true,g)}},n)}function createLoading(t){const i=type_loading();loadingCount.set(t,i);return i}function getLoadingKey(t){const i=srcBy$src.get(t);if(i===undefined){return t}if(loadingCount.has(i.id)){return i.id}const e=loadingCount.get(t);if(e===undefined){return i.id}loadingCount.delete(t);loadingCount.set(i.id,e);return i.id}function decLoading(t,i,e){if(i!==""){const n=e.get(i)-1;e.set(i,n);if(n<=0){toggleLoading(t,i,false,e)}}const n=e.get("")-1;e.set("",n);if(n>0){return}toggleLoading(t,"",false,e);for(const[o,r]of loadingCount){for(const[g,n]of r){if(n>0){continue}if(g===""){loadingCount.delete(o)}}}}function toggleLoading(t,i,e,n){const o=i===""?isFillingName:isFillingName+isFillingDiv+i;if(!e){t.removeAttribute(o,"");if(srcBy$src.get(t).isHide){t.content.firstChild.removeAttribute(o,"")}return}if(i===""){const r=n.get("");n.set("",r+1)}else{const r=n.get(i);if(r===undefined){n.set(i,1)}else{n.set(i,r+1)}}t.setAttribute(o,"");if(srcBy$src.get(t).isHide){t.content.firstChild.setAttribute(o,"")}}function type_loading(){return new Map([["",0]])}')],
