...
["/cmd/watch.js", import('data:text/javascript;text,let type_renderRes,p_target,srcBy$src,eval2,q_eval2,getProxy,kebabToCamelCase;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/render/render.js"),self.__i.get("/config.js"),self.__i.get("/descr.js"),self.__i.get("/eval2.js"),self.__i.get("/proxy.js"),self.__i.get("/str.js")]).then(arr=>{let m;m=arr[0];type_renderRes=m.type_renderRes;m=arr[1];p_target=m.p_target;m=arr[2];srcBy$src=m.srcBy$src;m=arr[3];eval2=m.eval2;q_eval2=m.q_eval2;m=arr[4];getProxy=m.getProxy;m=arr[5];kebabToCamelCase=m.kebabToCamelCase;}));export default{render(r){return eval2(r,r.$src,true).then(e=>watch_render(r,r.scope,getName(r),e))},q_render(a,c,i){return q_eval2(a,c,i).then(r=>{const t=getName(a),n=c.length,s=new Array(n);for(let e=0;e<n;e++){if(!i.has(e)){s[e]=watch_render(a,c[e].scope,t,r[e])}}return s})}};function getName(e){const r=e.reqCmd.args[0];if(r!==undefined&&r!==""){return kebabToCamelCase(r)}}function watch_render(e,r,t,n){const s=srcBy$src.get(e.$src).cache;if(!s.current.has(e.str)){s.current.set(e.str,type_watchCur())}const a=s.current.get(e.str);if(e.sync.renderParam.isLinking){s.isInits.add(e.str);a.watch=n;return type_renderRes(true)}if(s.isInits.has(e.str)){if(n===a.watch){return type_renderRes(true)}e.sync.onreadies.add(()=>{a.watch=n})}else{e.sync.onreadies.add(()=>{s.isInits.add(e.str);a.watch=n})}if(t!==undefined){r[p_target][t]=getProxy(n)}return null}function type_watchCur(){return{watch:undefined}}')],
...
let syncInRender,render,setDelay,renderLoop,addAnimation,globVarName,locVarName,viewVarName,$srcById,isAnimationVisible,getLoc,setLoc,getProxy,getView,setView;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/addons.js"),self.__i.get("/api.js"),self.__i.get("/render/algo.js"),self.__i.get("/config.js"),self.__i.get("/descr.js"),self.__i.get("/dom.js"),self.__i.get("/loc.js"),self.__i.get("/proxy.js"),self.__i.get("/view.js")]).then(arr=>{let m;m=arr[2];syncInRender=m.syncInRender;render=m.render;setDelay=m.setDelay;renderLoop=m.renderLoop;addAnimation=m.addAnimation;m=arr[3];globVarName=m.globVarName;locVarName=m.locVarName;viewVarName=m.viewVarName;m=arr[4];$srcById=m.$srcById;m=arr[5];isAnimationVisible=m.isAnimationVisible;m=arr[6];getLoc=m.getLoc;setLoc=m.setLoc;m=arr[7];getProxy=m.getProxy;m=arr[8];getView=m.getView;setView=m.setView;}));self.__imports.then(()=>{delete self.__i;delete self.__p;delete self.__imports;setTimeout(()=>{(()=>{const e=import.meta.url;self.mw_debugLevel=e.indexOf("debug=1")!==-1?1:e.indexOf("debug=2")!==-1?2:0;self[globVarName]=getProxy(self[globVarName]||{});self[locVarName]=getProxy(getLoc(location.href));self[viewVarName]=getProxy(getView(document));if(e.indexOf("load=skip")!==-1){if(self.mw_debugLevel!==0){import(e.replace(/([^\/]+?\.js)/,"getlineno.js"))}return}const t=()=>{if(self.mw_debugLevel===0){render(undefined,undefined,undefined,e.indexOf("linking=1")!==-1);return}import(e.replace(/([^\/]+?\.js)/,"getlineno.js")).then(e=>e.default).then(()=>render(undefined,undefined,undefined,e.indexOf("linking=1")!==-1))};if(e.indexOf("load=onload")===-1){if(document.readyState==="interactive"||document.readyState==="complete"){t();return}document.addEventListener("DOMContentLoaded",t,{once:true});return}if(document.readyState==="complete"){t();return}self.addEventListener("load",t,{once:true})})();const view=self[viewVarName],$scroll=document.scrollingElement,evtOpt={passive:true};if($scroll.clientWidth===980){setTimeout(()=>setView(document),500)}self.addEventListener("scroll",()=>{const e=new Set,t=new Set;for(const n of syncInRender){if(n.stat!==0||n.scrollAnimations.size===0){continue}const o=new Set;for(const i of n.scrollAnimations){if(!$srcById.has(i.viewedSrcId)){n.scrollAnimations.delete(i);if(n.scrollAnimations.size===0){t.add(n)}continue}if(isAnimationVisible(i)){n.scrollAnimations.delete(i);o.add(i)}}if(o.size!==0){e.add(addAnimation(n,o,true));t.add(n)}}if(e.size!==0){Promise.all(e).then(()=>renderLoop(t));return}if(t.size!==0){console.warn("2animation");renderLoop(t)}setDelay(10,()=>{view.scrollTop=$scroll.scrollTop;view.scrollLeft=$scroll.scrollLeft})},evtOpt);self.addEventListener("resize",()=>{setDelay(100,()=>setView(document))},evtOpt);self.addEventListener("popstate",()=>{setLoc(location.href)},evtOpt);}, 0)})