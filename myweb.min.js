...
["/api.js", import('data:text/javascript;text,let incCache,render,curRender,setDelay,p_target,p_topUrl,removeChild,dispatchCustomEvent,setLoc,getProxy,oset,del,copyToClipboard,getUrl;self.__p.add(import.meta.__imports=Promise.all([self.__i.get("/cmd/inc.js"),self.__i.get("/render/algo.js"),self.__i.get("/config.js"),self.__i.get("/dom.js"),self.__i.get("/evt.js"),self.__i.get("/loc.js"),self.__i.get("/proxy.js"),self.__i.get("/oset.js"),self.__i.get("/str.js"),self.__i.get("/url.js")]).then(arr=>{let m;m=arr[0];incCache=m.incCache;m=arr[1];render=m.render;curRender=m.curRender;setDelay=m.setDelay;m=arr[2];p_target=m.p_target;p_topUrl=m.p_topUrl;m=arr[3];removeChild=m.removeChild;m=arr[4];dispatchCustomEvent=m.dispatchCustomEvent;m=arr[5];setLoc=m.setLoc;m=arr[6];getProxy=m.getProxy;m=arr[7];oset=m.oset;del=m.del;m=arr[8];copyToClipboard=m.copyToClipboard;m=arr[9];getUrl=m.getUrl;}));function begin(){self.mw_incCache=incCache;self.mw_render=render;self.mw_curRender=curRender;self.mw_setDelay=setDelay;self.mw_p_target=p_target;self.mw_p_topUrl=p_topUrl;self.mw_removeChild=removeChild;self.mw_setLoc=setLoc;self.mw_getProxy=getProxy;self.mw_oset=oset;self.mw_del=del;self.mw_copyToClipboard=copyToClipboard;self.mw_getUrl=getUrl;self.mw_dispatchCustomEvent=dispatchCustomEvent}if(self.__imports===undefined){begin()}else{self.__imports.then(begin)}')],
