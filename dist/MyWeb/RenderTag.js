...
export class RenderTag extends RenderLoop {
    async renderTag($src, scope, str, sync) {
        if (sync.stat !== 0) {
    ...
    renderAfterAttr($src, scope, sync, src) {
        console.log(3333, $src, src);
        const nameIs = $src.getAttribute("is");
        const el = sync.local.get(src.id).customElementByName.get(nameIs !== null ? nameIs.toUpperCase() : $src.nodeName); //!!
    ...
    async renderChildren($src, scope, sync) {
    ...
    createReq($src, str, expr, scope, event, sync) {
        return new Req($src, str, expr, scope, event, sync, this.context.commandWithArgsByStr.get(str)); //<- commandWithArgsByStr set in createAttr
    }
}
