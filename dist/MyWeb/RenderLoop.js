...
export class RenderLoop extends Loading {
    checkScrollAnimations() {
        const pArr = [];
        const scrollSync = new Set();
    ...
    isAddScrollAnimationsEvent = new WeakSet();
    ...
    testLocalEventsBySrcId(local, srcId) {
        const l = local.get(srcId); //!!
        if (l.animationsCount === 0) {
            this.dispatchLocalEventsBySrcId(srcId, l);
        }
    }
    addAnimation(sync, tasks, isSet) {
        //todo
        if (sync.stat !== 0) {
        ...
        const nows = isSet ? tasks : new Set();
    ...
    dispatchLocalEventsBySrcId(srcId, l) {
