...
export class Render extends Q_renderTag {
    ...
    //todo ??
    get$srcScope($e) {
        const srcBy$src = this.context.srcBy$src, $top = this.context.rootElement.parentNode;
        for (let $i = $e; $i !== $top; $i = $i.parentNode) {
            //!!
    ...
    runRender(delay = this.context.delayInMs, srcId) {
    ...
    renderRenderParams(delayParams) {
        const toCancelSync = new Set();
        ...
        if (this.context.syncInRender.size !== 0) {
            ...
            const s = new Set(); //для того: если были уже отменены синки и мы их пустим на проверку то получится, что новые параметры будут удалены из-за условия curStat !== 0
            for (const sync of this.context.syncInRender) {
                if (sync.stat === 0) {
            ...
            for (const sync of s) {
                for (const [dId, r] of byDescr) {
                    ...
                    if (stat === 3) {
                        //new below ниже
                        repeatByD.set(this.context.srcById.get(sync.renderParam.srcId).descr.id, sync.renderParam); //!!
                        continue;
                    }
                    //new above
                    //todo rem
        ...
        if (byDescr.size !== 0) {
            ...
            this.context.currentRender = this.context.currentRender.then(() => this.renderByRenderParamsGroupedByDescr(repeatByD, delayParams));
            //} else {
            //	return this.context.currentRender;
        }
        //if (delayP.size === 0) {
    ...
    //todo rename
    renderByRenderParamsGroupedByDescr(byDescr, delayParams) {
        if (my.debugLevel !== 0) {
            const sIds = new Set();
        ...
        for (const [dId, r] of byDescr) {
            ...
            const $sync = this.context.$srcById.get(r.srcId); //!!
            ...
            if (arrLen === 1) {
                ...
                pArr.push(this.renderTag($sync, r.scope, r.str, sync));
                continue;
            }
            const arr = new Array(arrLen);
            let i = 0;
            for (const sId of r.srcIds) {
                arr[i++] = new Q_arr(this.context.$srcById.get(sId), r.scope, null); //!!
            }
            //todo не могу сообразить, почему этот (1) вариант быстрее! неужели выполняется в нескольких потоках?
            //console.time(111);
            pArr.push(this.q_renderByRenderParams(r, sync, arr));
        ...
        return Promise.all(pArr)
            ...
            for (const sync of syncInThisRender) {
                ...
                sync.promise.then(() => {
                    ...
                    const sIds = sync.renderParam.srcIds.size === 0
...
alert(1)
    ...
    q_renderByRenderParams(renderParam, sync, arr) {
        const nows = [];
        const deferreds = [];
        ...
        if (nows.length !== 0) {
            //console.log(1, nows)
            return this.q_renderTag(nows, renderParam.str, new Set(), sync).then(() => deferreds.length !== 0 &&
                sync.afterAnimations.add(new Task(() => this.q_renderByRenderParams(renderParam, sync, deferreds), sync.local, 0)));
        }
        //console.log(2, deferreds)
    ...
    // 	}
    // }
    prepareRenderParam(toCancelSync) {
        ...
        for (const [dId, r] of byDescr) {
            ...
            for (let f = true, i = 0; i < $elsLen; ++i) {
                ...
                /*
                if (f && iSrc.isCmd) {//!!эта оптимизация не имеет особого преимущества
                    f = false;
                    r.srcId = iSrc.id;
        ...
        for (const [dId, r] of byDescr) {
            ...
            let $i = $srcById.get(r.srcId); //!!
            let iSrc = srcBy$src.get($i); //!! //todo by id
...
console.log(444, mJ.len - mI.len, $l, $srcById.get(byDescr.get(iDId).srcId), descrById.get(jDId), byDescr.get(jDId));
                    ...
                    const $p = $srcById.get(byDescr.get(iDId).srcId).parentNode; //!!
...
//				if (mI.len > mJ.len && mI.descrId[jDId]) {
    ...
    prpDeleteDescrId(byDescr, descrId, toCancelSync) {
        if (!byDescr.has(descrId)) {
            return;
        }
        const s = byDescr.get(descrId).srcIds; //!!
        //let sId;
        for (const sync of this.context.syncInRender) {
    ...
    getPosStat(sync, newRenderParam) {
        ...
        for (let i, $i = $new.parentNode; $i !== $top; $i = $i.parentNode) {
            //!!
            const iSrc = srcBy$src.get($i); //!!
